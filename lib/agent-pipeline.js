 ...
 *   optional context refinement -> N concurrent completions -> majority vote
...
function spawnProgress(deps, problem, candidate, agentConfig, logPath) {
  ...
  Promise.resolve()
    ...
    .then(job => logPath && updatePipelineLog(logPath, {
      progressMonitor: job.status === 'done'
        ? { enabled: true, ...job.result }
        : { enabled: true, status: job.status, error: job.error },
    }))
    .catch(error => logPath && updatePipelineLog(logPath, {
      progressMonitor: { enabled: true, error: error?.message ?? String(error) },
    }))
}
...
export async function* turboAgentStream(ctx, original, agent, next, deps = {}) {
  ...
  const candidates = await Promise.all(entries.map(entry =>
    callOnce(ctx, candidateRequest(base, entry), entry)))
  original.signal?.throwIfAborted?.()
  ...
  const log = {
    ...
    verifier: selection,
    bestIndex: candidates.indexOf(winner),
    bestModel: winner.entry,
    elapsedMs,
    progressMonitor: { enabled: Boolean(agentConfig.progressMonitor?.enabled), status: 'pending' },
  }
  let logPath = ''
  try { if (saveLog) logPath = await saveLog(log) } catch { /* observability only */ }
  spawnProgress({ track }, problem, winner, agentConfig, logPath)
