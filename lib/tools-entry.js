...
export const VERIFIER_TOOLS_GUIDANCE =
  '当前会话处于 Verifier 模式（LLM-as-a-Verifier / TurboAgent 模式）：每个普通对话模型 step 都由传输层自动执行“可选 context refinement → N 个并行或逐路串行候选 → 多数投票或 PPT 评审 → 只返回最佳候选 → 异步进度评分”，不依赖主 Agent 主动调用工具。' +
